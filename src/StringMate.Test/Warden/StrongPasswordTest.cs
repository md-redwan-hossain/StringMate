...
public class StrongPasswordTest
{
    ...
    public void IsStrongPassword() =>
        PasswordData.Valid.All(x => Validation.Warden.IsStrongPassword(x)).ShouldBeTrue();

    [Fact]
    public void IsNotStrongPassword() =>
        PasswordData.Invalid.All(x => Validation.Warden.IsStrongPassword(x)).ShouldBeFalse();
